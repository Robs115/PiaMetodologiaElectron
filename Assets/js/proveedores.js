...
function renderTabla(lista) {
    ...
    if (lista.length === 0) {
        ...
        fila.innerHTML = `<td colspan="7" style="text-align: center;">No hay proveedores registrados</td>`;
    ...
    lista.forEach(proveedor => {
        ...
        // Formatear el campo activo para mostrarlo mejor
        const activoTexto = proveedor.activo == 1 ? "Activo" : "Inactivo";
        const activoClass = proveedor.activo == 1 ? "activo" : "inactivo";
        ...
        fila.innerHTML = `
            ...
            <td class="${activoClass}">${activoTexto}</td>
            <td class="acciones">
                <button class="btn btn-editar" onclick="editarProveedor(${proveedor.id})">
                    Editar
                </button>
                <button class="btn btn-eliminar" onclick="eliminarProveedor(${proveedor.id})">
                    Eliminar
                </button>
            </td>
        `;
...
window.editarProveedor = async (id) => {
    try {
        window.location.href = `/proveedores/editar/${id}`;
    } catch (error) {
        console.error('Error:', error);
...
window.eliminarProveedor = async (id) => {
    const confirmado = await mostrarConfirmacion(
        ...
        'Esta acción no se puede deshacer. Si tiene productos asociados, no podrá eliminarlo.'
    );
    ...
    try {
        ...
        mostrarNotificacion(resultado.message || 'Proveedor eliminado correctamente', 'success');
        await cargarProveedores();
        
    } catch (error) {
        console.error('Error:', error);
        mostrarNotificacion(error.message || 'Error al eliminar proveedor', 'error');
    }
};
